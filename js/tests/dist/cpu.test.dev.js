...
test('ADC #$68 Adds to accumulator (0x01) to make 0x69 adding the carry, unsetting C flag once consumed', function () {
...
test('ADC #$68 Adds to accumulator (0xFE) to make 0x69 adding the carry, resetting C flag', function () {
...
test('ADC $80 gets 0x68 from ZP$80 and adds to accumulator (0x01) to make 0x69', function () {
...
test('ADC $80,X gets 0x68 from $82 and adds to accumulator (0x01) to make 0x69 if X == $02', function () {
...
test('ADC $2200 gets 0x69 from $2200 and adds to accumulator (0x01) to make 0x69', function () {
...
test('ADC $2200,X gets 0x69 from $220F and adds to accumulator (0x01) to make 0x69 if X == #$0F', function () {
...
test('ADC $2200,Y gets 0x69 from $220F and adds to accumulator (0x01) to make 0x69 if Y == #$0F', function () {
...
test('ADC ($20,X) gets 0x68 from $3074 and adds to accumulator (0x01) to make 0x69 if X == 0x04', function () {
...
test('ADC ($0),Y gets 0x68 from $0310 and adds to accumulator (0x01) to make 0x69 if Y == 0x90', function () {
...
test('AND $80 gets 0x69 from ZP$80 and performs a logical AND on accumulator (0xE9) to make 0x69', function () {
...
test('AND $80,X gets 0x68 from $82 performs a logical AND on accumulator (0xE9) to make 0x69 if X == $02', function () {
...
test('AND $2200 gets 0x69 from $2200 and performs logical AND on accumulator (0xE9) to make 0x69', function () {
...
test('AND $2200,X gets 0x69 from $220F and performs logical AND on accumulator (0xE9) to make 0x69 if X == #$0F', function () {
...
test('AND $2200,Y gets 0x69 from $220F and performs logical AND on accumulator (0xE9) to make 0x69 if Y == #$0F', function () {
...
test('ASL A shifts accumulator (0x80) left 1 bit setting the C flag and resulting in 0x00', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ASL_A]));
  cpu.A = 0x80;
  cpu.execute();
  expect(cpu.A).toBe(0x00);
...
test('BCC skips setting A to 0 because Carry is clear, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BCC_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
  cpu.A = 0x69;
  cpu.X = 0x00;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
...
test('BCS skips setting A to 0 because Carry is set, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BCS_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
  cpu.set_flag(_flag["default"].C);
  cpu.A = 0x69;
  cpu.X = 0x00;
  cpu.execute();
...
test('BEQ skips setting A to 0 because Zero flag is set, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BEQ_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
  cpu.set_flag(_flag["default"].Z);
  cpu.A = 0x69;
  cpu.X = 0x00;
  cpu.execute();
...
test('BMI skips setting A to 0 because Negative flag is set, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BMI_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
...
test('BNE skips setting A to 0 because Zero flag is clear, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BNE_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
  cpu.A = 0x69;
  cpu.X = 0x00;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
...
test('BPL skips setting A to 0 because Negative flag is clear, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BPL_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
  cpu.A = 0x69;
  cpu.X = 0x00;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
...
test('BVC skips setting A to 0 because Overflow flag clear, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BVC_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
  cpu.A = 0x69;
  cpu.X = 0x00;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
...
test('BVS skips setting A to 0 because Overflow flag is set, but X still set to 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].BVS_REL, 0x82, _instruction["default"].LDA_IM, 0x00, _instruction["default"].LDX_IM, 0x69]));
  cpu.set_flag(_flag["default"].V);
  cpu.A = 0x69;
  cpu.X = 0x00;
  cpu.execute();
...
test('EOR $80 gets 0x69 from ZP$80 and performs a Exclusive OR on accumulator (0xE9) to make 0x80', function () {
...
test('EOR $80,X gets 0x68 from $82 performs a Exclusive OR on accumulator (0xE9) to make 0x80 if X == $02', function () {
...
test('EOR $2200 gets 0x69 from $2200 and performs Exclusive OR on accumulator (0xE9) to make 0x80', function () {
...
test('EOR $2200,X gets 0x69 from $220F and performs Exclusive OR on accumulator (0xE9) to make 0x80 if X == #$0F', function () {
...
test('EOR $2200,Y gets 0x69 from $220F and performs Exclusive OR on accumulator (0xE9) to make 0x80 if Y == #$0F', function () {
...
test('LSR A shifts accumulator (0x30) right 1 bit halving the value to 0x18, C Flag not set', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].LSR_A]));
  cpu.A = 0x30;
  cpu.execute();
  expect(cpu.A).toBe(0x18);
  expect(cpu.check_flag(_flag["default"].C)).toBe(false);
});
test('LSR A shifts accumulator (0x01) right 1 bit setting the C flag and resulting in 0x00', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].LSR_A]));
  cpu.A = 0x01;
  cpu.execute();
  expect(cpu.A).toBe(0x00);
...
test('LSR $80 shifts ZP$80 (0x30) right 1 bit halving the value to 0x60', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].LSR_ZP, 0x80]));
  cpu.store_byte(0x80, 0x30);
  cpu.execute();
  expect(cpu.read_byte(0x80)).toBe(0x18);
});
test('LSR $80,X shifts ZP$82 (0x30) right 1 bit halving the value to 0x60, if X == 02', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].LSR_ZPX, 0x80]));
  cpu.store_byte(0x82, 0x30);
  cpu.X = 0x02;
  cpu.execute();
  expect(cpu.read_byte(0x82)).toBe(0x18);
});
test('LSR $2200 shifts $2200 (0x30) right 1 bit halving the value to 0x60', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].LSR_ABS, 0x00, 0x22]));
  cpu.store_byte(0x2200, 0x30);
  cpu.execute();
  expect(cpu.read_byte(0x2200)).toBe(0x18);
});
test('LSR $2200,X shifts $220F (0x30) right 1 bit halving the value to 0x60 if X == 0F', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].LSR_ABSX, 0x00, 0x22]));
  cpu.store_byte(0x220F, 0x30);
  cpu.X = 0x0F;
  cpu.execute();
...
test('ORA #$69 performs logical ORA on accumulator (0x29) to make 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_IM, 0x69]));
  cpu.A = 0x29;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
});
test('ORA $80 gets 0x69 from ZP$80 and performs a logical ORA on accumulator (0x29) to make 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_ZP, 0x80]));
  cpu.store_byte(0x80, 0x69);
  cpu.A = 0x29;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
});
test('ORA $80,X gets 0x68 from $82 performs a logical ORA on accumulator (0x29) to make 0x69 if X == $02', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_ZPX, 0x80]));
  cpu.store_byte(0x82, 0x69);
  cpu.A = 0x29;
  cpu.X = 0x02;
  cpu.execute();
...
test('ORA $2200 gets 0x69 from $2200 and performs logical ORA on accumulator (0x29) to make 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_ABS, 0x00, 0x22]));
  cpu.store_byte(0x2200, 0x69);
  cpu.A = 0x29;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
});
test('ORA $2200,X gets 0x69 from $220F and performs logical ORA on accumulator (0x29) to make 0x69 if X == #$0F', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_ABSX, 0x00, 0x22]));
  cpu.store_byte(0x220F, 0x69);
  cpu.A = 0x29;
  cpu.X = 0x0F;
  cpu.execute();
...
test('ORA $2200,Y gets 0x69 from $220F and performs logical ORA on accumulator (0x29) to make 0x69 if Y == #$0F', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_ABSY, 0x00, 0x22]));
  cpu.store_byte(0x220F, 0x69);
  cpu.A = 0x29;
  cpu.Y = 0x0F;
  cpu.execute();
...
test('ORA ($20,X) gets 0x69 from $3074 and performs a logical ORA on accumulator (0x29) to make 0x69 if X == 0x04', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_INDX, 0x20]));
  cpu.store_byte(0x3074, 0x69);
  cpu.store_byte(0x24, 0x74);
  cpu.store_byte(0x25, 0x30);
  cpu.A = 0x29;
...
test('ORA ($0),Y gets 0x69 from $0310 and performs a logical ORA on accumulator (0x29) to make 0x69 if Y == 0x90', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ORA_INDY, 0x00]));
  cpu.store_byte(0x0310, 0x69);
  cpu.store_byte(0x00, 0x80);
  cpu.store_byte(0x01, 0x02);
  cpu.Y = 0x90;
...
test('ROL A shifts accumulator (0x30) left 1 bit doubling the value to 0x60, then setting bit 0 to old Carry flag, C Flag not set', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROL_A]));
  cpu.set_flag(_flag["default"].C);
  cpu.A = 0x30;
  cpu.execute();
...
test('ROL A shifts accumulator (0x80) right 1 bit setting the C flag and resulting in 0x00', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROL_A]));
  cpu.A = 0x80;
  cpu.execute();
  expect(cpu.A).toBe(0x00);
...
test('ROL $80 shifts ZP$80 (0x30) right 1 bit doubling the value to 0x60', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROL_ZP, 0x80]));
  cpu.store_byte(0x80, 0x30);
  cpu.execute();
  expect(cpu.read_byte(0x80)).toBe(0x60);
});
test('ROL $80,X shifts ZP$82 (0x30) right 1 bit doubling the value to 0x60, if X == 02', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROL_ZPX, 0x80]));
  cpu.store_byte(0x82, 0x30);
  cpu.X = 0x02;
  cpu.execute();
  expect(cpu.read_byte(0x82)).toBe(0x60);
});
test('ROL $2200 shifts $2200 (0x30) right 1 bit doubling the value to 0x60', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROL_ABS, 0x00, 0x22]));
  cpu.store_byte(0x2200, 0x30);
  cpu.execute();
  expect(cpu.read_byte(0x2200)).toBe(0x60);
});
test('ROL $2200,X shifts $220F (0x30) right 1 bit doubling the value to 0x60 if X == 0F', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROL_ABSX, 0x00, 0x22]));
  cpu.store_byte(0x220F, 0x30);
  cpu.X = 0x0F;
  cpu.execute();
  expect(cpu.read_byte(0x220F)).toBe(0x60);
...
test('ROR A shifts accumulator (0x30) right 1 bit halving the value to 0x18, C Flag not set', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROR_A]));
  cpu.set_flag(_flag["default"].C);
  cpu.A = 0x30;
  cpu.execute();
...
test('ROR A shifts accumulator (0x01) right 1 bit setting the C flag and resulting in 0x00', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROR_A]));
  cpu.A = 0x01;
  cpu.execute();
  expect(cpu.A).toBe(0x00);
...
test('ROR $80 shifts ZP$80 (0x30) right 1 bit halving the value to 0x60', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROR_ZP, 0x80]));
  cpu.store_byte(0x80, 0x30);
  cpu.execute();
  expect(cpu.read_byte(0x80)).toBe(0x18);
});
test('ROR $80,X shifts ZP$82 (0x30) right 1 bit halving the value to 0x60, if X == 02', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROR_ZPX, 0x80]));
  cpu.store_byte(0x82, 0x30);
  cpu.X = 0x02;
  cpu.execute();
  expect(cpu.read_byte(0x82)).toBe(0x18);
});
test('ROR $2200 shifts $2200 (0x30) right 1 bit halving the value to 0x60', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROR_ABS, 0x00, 0x22]));
  cpu.store_byte(0x2200, 0x30);
  cpu.execute();
  expect(cpu.read_byte(0x2200)).toBe(0x18);
});
test('ROR $2200,X shifts $220F (0x30) right 1 bit halving the value to 0x60 if X == 0F', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].ROR_ABSX, 0x00, 0x22]));
  cpu.store_byte(0x220F, 0x30);
  cpu.X = 0x0F;
  cpu.execute();
  expect(cpu.read_byte(0x220F)).toBe(0x18);
...
test('SBC #$6A substracts 0x01 from accumulator (0x01) to make 0x01, unsetting C flag once consumed', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_IM, 0x01]));
  cpu.A = 0x6A;
  cpu.set_flag(_flag["default"].C);
  cpu.execute();
  expect(cpu.A).toBe(0x69);
...
test('SBC #$6A substracts 0x01 from accumulator (0x02) with the Not of C flag to make 0xFF, setting C flag', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_IM, 0x01]));
  cpu.A = 0x01;
  cpu.execute();
...
test('SBC #$6A substracts 0x01 from accumulator (0x01) to make 0x00, setting Z flag', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_IM, 0x01]));
  cpu.A = 0x01;
  cpu.set_flag(_flag["default"].C);
  cpu.execute();
...
test('SBC $80 gets 0x01 from ZP$80 and substracts from accumulator (0x6A), with C̅ to make 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_ZP, 0x80]));
  cpu.store_byte(0x80, 0x01);
  cpu.A = 0x6B;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
});
test('SBC $80,X gets 0x01 from $82 and substracts from accumulator (0x6A) with C̅ to make 0x69 if X == $02', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_ZPX, 0x80]));
  cpu.store_byte(0x82, 0x01);
  cpu.A = 0x6B;
  cpu.X = 0x02;
  cpu.execute();
...
test('SBC $2200 gets 0x01 from $2200 and substracts from accumulator (0x6B) with C̅ to make 0x69', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_ABS, 0x00, 0x22]));
  cpu.store_byte(0x2200, 0x01);
  cpu.A = 0x6B;
  cpu.execute();
  expect(cpu.A).toBe(0x69);
});
test('SBC $2200,X gets 0x01 from $220F and substracts from accumulator (0x6B) with C̅ to make 0x69 if X == #$0F', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_ABSX, 0x00, 0x22]));
...
test('SBC $2200,Y gets 0x01 from $220F and substracts from accumulator (0x6B) with C̅ to make 0x69 if Y == #$0F', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_ABSY, 0x00, 0x22]));
  cpu.store_byte(0x220F, 0x01);
  cpu.A = 0x6B;
  cpu.Y = 0x0F;
  cpu.execute();
...
test('SBC ($20,X) gets 0x01 from $3074 and substracts from accumulator (0x6B) with C̅ to make 0x69 if X == 0x04', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_INDX, 0x20]));
...
test('SBC ($0),Y gets 0x01 from $0310 and substracts from accumulator (0x6B) with C̅ to make 0x69 if Y == 0x90', function () {
  var cpu = get_CPU(0xF000, new Uint8Array([_instruction["default"].SBC_INDY, 0x00]));
